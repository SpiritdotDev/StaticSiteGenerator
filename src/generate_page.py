...
def generate_pages_recursive(src_path, template_path, dest_path):
    ...
    for path in to_visit:
        if os.path.isdir(os.path.join(src_path, path)):
            generate_pages_recursive(os.path.join(src_path,path),template_path,os.path.join(dest_path,path))
        elif os.path.isfile(os.path.join(src_path,path)):
            files_to_gen.append(path)
    for file in files_to_gen:
        generate_page(os.path.join(src_path,file), template_path, Path(os.path.join(dest_path,file)).with_suffix(".html"))
